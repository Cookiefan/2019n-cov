...
beijing.to_csv('beijing_截至{}_24时.csv'.format(datetime.date.today().strftime('%m%d')), index=None)

shanghai = df.loc[(df['provincename'] == '上海') & (df['cityname'] == '全市')]
shanghai = shanghai[['confirmed', 'suspected', 'dead', 'cured', 'time']]
shanghai.to_csv('shanghai_截至{}_24时.csv'.format(datetime.date.today().strftime('%m%d')), index=None)
